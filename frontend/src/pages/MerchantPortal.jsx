...
function Address({ text, language, value = {} }) {
  return <fieldset className="merchant-address"><legend>{text.address}</legend>{fields.filter(f => f !== 'country').map(f => <Input key={f} name={'address.'+f} label={text[f]} value={value?.[f]} maxLength={150}/>)}<div className="merchant-field"><label htmlFor="merchant-country">{text.country}</label><select id="merchant-country" name="address.country" defaultValue={String(value?.country || '').trim().toUpperCase()}><option value="">{text.countryPlaceholder}</option>{countryOptions(language, value?.country).map(c => <option key={c.value} value={c.value}>{c.label}</option>)}</select></div></fieldset>;
}
function Portal() {
      ...
      {!ready ? <p role="status">{t.loading}</p> : authPage ? <section className="merchant-auth">
        ...
        {section === 'products' && <><button onClick={() => startEdit('product')}>{t.newProduct}</button>{!data.products.length && <p>{t.emptyProducts}</p>}<div className="merchant-cards">{data.products.map(p => {
          ...
          return <article key={p.productId}>{p.image && <img className="merchant-product-image" src={p.image} alt="" loading="lazy" referrerPolicy="no-referrer"/>}<h2>{p.name}</h2><p className="merchant-help">{t.boundOffers}: {active.length} · {t.stock}: {active.reduce((n,o) => n+o.stockQuantity,0)} · {t.displayCount}: {count}</p>
            ...
            <h3>{t.offers}</h3>{!offers.length && <p>{t.emptyOffers}</p>}{offers.map(o => <div className="merchant-offer" key={o.offerId}><p><strong>{money(o)}</strong> · {data.locations.find(l => l.locationId === o.locationId)?.name}<br/>{t.stock}: {o.stockQuantity} · {t.availableStock}: {o.availableStock ?? o.stockQuantity} · {t.heldStock}: {o.reservedQuantity ?? 0} · {o.active ? t.ACTIVE : t.INACTIVE}</p><button className="merchant-quiet" onClick={() => startEdit('offer',o)}>{t.edit}</button></div>)}
          </article>;
        })}</div></>}
        {section === 'devices' && <>{!data.devices.length && <p>{t.emptyDevices}</p>}<div className="merchant-cards">{data.devices.map(d => <article key={d.deviceId}><span className={'merchant-badge '+(d.online ? 'is-online' : '')}>{d.online ? t.online : t.offline}</span><h2>{d.displayName}</h2><p>{d.location.name}</p><h3>{d.product?.name || t.unassigned}</h3>{d.offer && <p>{money(d.offer)} · {t.stock}: {d.offer.stockQuantity} · {t.availableStock}: {d.offer.availableStock ?? d.offer.stockQuantity}</p>}<p>{t[d.assignmentStatus]}</p>{d.offer?.active === false && <p>{t.PAUSED}</p>}<p className="merchant-help">{t.lastSeen}: {d.lastSeenAt ? new Date(d.lastSeenAt).toLocaleString(language) : t.none}<br/>{t.firmware}: {d.firmwareVersion || t.none}</p><div className="merchant-actions"><button className="merchant-quiet" onClick={() => startEdit('device',d)}>{t.rename}</button><Link className="merchant-button" to={'/binding/'+d.deviceId}>{t.connect}</Link></div><details><summary>{t.technical}</summary><p>{d.deviceId}</p></details></article>)}</div><p className="merchant-help">{t.bindHelp}</p></>}
        {section === 'locations' && <><button onClick={() => startEdit('location')}>{t.newLocation}</button><div className="merchant-cards">{data.locations.map(l => <article key={l.locationId}><h2>{l.name}</h2><p>{addressLabel(l.address, language)}</p><p>{t[l.status]}</p><button className="merchant-quiet" onClick={() => startEdit('location',l)}>{t.edit}</button></article>)}</div></>}
        {section === 'settings' && <article><h2>{data.merchant.displayName}</h2><p>{data.merchant.contactEmail}</p><p>{data.merchant.phone}</p><p>{addressLabel(data.merchant.address, language)}</p><p>{t.status}: {t[data.merchant.status]}</p><button onClick={() => startEdit('profile',data.merchant)}>{t.edit}</button></article>}
        ...
        {edit && <section className="merchant-editor" key={edit.type+(edit.item.productId || edit.item.locationId || edit.item.deviceId || '')+(edit.item.offerId || '')}>
          ...
          <form onSubmit={save}><fieldset disabled={busy}>
            ...
            {edit.type === 'location' && <><Input name="name" label={t.locationName} value={edit.item.name} required maxLength={120}/><Address text={t} language={language} value={edit.item.address}/></>}
            {edit.type === 'profile' && <><Input name="displayName" label={t.business} value={edit.item.displayName} required maxLength={120}/><Input name="contactEmail" label={t.contactEmail} value={edit.item.contactEmail} type="email" required maxLength={254}/><Input name="phone" label={t.phone} value={edit.item.phone} type="tel" maxLength={40}/><Address text={t} language={language} value={edit.item.address}/></>}
