...
export async function reservationFixture(origin = 'http://127.0.0.1:5178') {
  ...
  await Promise.all([Merchant,Location,MerchantProduct,Offer,MerchantReservation,MerchantAccount].map(m=>m.init()));
  ...
  const device=createDeviceService({now});
  ...
  return {app,service,device,offer,now,password,advance:async ms=>{offset+=ms;await expireReservations(now());},
