...
fixture.app.get('/__test__/notify/:id',async(req,res)=>{const items=await AvailabilitySubscription.find({offerId:req.params.id}).lean();res.json({statuses:items.map(i=>i.status),mails:fixture.messages.filter(m=>m.text.includes('/o/'+req.query.publicId)).length});});
