...
Hardware-Synchronisierung Stufe 3 ist auf `main` committed und im aktiven Environment `esp32dev_spi_cs5_rst4_app` durch neun statische Vertragsprüfungen sowie einen grünen Build bestätigt. Die Firmware pollt die gekoppelte Demo-Hardware-Projektion alle drei Sekunden in einem Hintergrund-Task, authentifiziert `demo-device` per lokalem Geräte-Secret und validiert TLS nach nicht blockierender NTP-Zeitsynchronisierung über den offiziellen selbstsignierten `GTS Root R4`. Dieser Trust-Anchor-Fix wurde auf echter Hardware bestätigt; der anschließend mit rotiertem Geräte-Secret erzeugte Build ist noch nicht auf den ESP32 geflasht. WLAN-Fallback und Runtime-Reconnect behalten den letzten gültigen Displayzustand. `bound:false`, Produktwechsel und alle Demo-Status werden backendgeführt gerendert; QR-Codes erscheinen nur für `READY`, `CHECKOUT_STARTED` und `CANCELLED` und wählen ihre Version längenabhängig bis Version 10. Der bestätigte Build mit lokal konfiguriertem Geräte-Secret nutzt 47.980 Byte RAM und 972.949 Byte Flash.
...
Hardware-Synchronisierung Stufe 1 bis 3 wurde mit Commit `8de5efa` auf `main` veröffentlicht. Die DigitalOcean-App `qr-backend` deployt daraus `qr2buy-backend` und `qr2buy-frontend`; beide Komponenten waren am 4. September 2026 ACTIVE. Die drei Hardware-ENV-Namen sind ausschließlich am Backend-Service gesetzt, `DEMO_PUBLIC_BASE_URL` ist `https://qr2buy.com`. Der Live-Smoke bestätigte Frontpage und Health mit HTTP 200, neue DemoSession und unveränderte `READY`-Ausgangszustände, den Hardware-Endpunkt ohne Geräte-Secret mit 401 sowie mit Geräte-Secret vor der Kopplung mit `{ ok: true, bound: false }`. Das autorisierte Live-Binding projizierte `bag`, danach per PATCH `tree` und abschließend wieder `bag`; die QR-Ziele verwiesen jeweils exakt auf dieselbe DemoSession. Es wurden weder Reservierung noch Zahlung ausgelöst. Lokale ENV- und Firmware-Secret-Dateien bleiben ignoriert.

Security-/TLS-Nachlauf vom 4. September 2026: Commit `a7bca06` redigiert Authorization-, Pairing- und Device-Header defensiv, serialisiert Responses ohne eingebettetes Requestobjekt und korrigiert das HTTP-Loglevel. Der Fix war vor der Rotation produktiv ACTIVE; kontrollierte Requests bestätigten, dass weder alter noch neuer Device-Wert in den neuen Runtime-Logs erscheint. Pairing- und Device-Secret wurden danach lokal und im DigitalOcean-Backend rotiert, während `DEMO_HARDWARE_ENCRYPTION_KEY` unverändert blieb. Der Hardware-Endpunkt antwortete ohne Secret und mit dem alten Device-Secret jeweils 401, mit dem neuen Wert 200. Backend und Firmware besitzen bytegleich das neue Device-Secret in ausschließlich ignorierten lokalen Dateien. Der Frontend-Regressionsnachweis bestätigt `bag → tree → bag` mit genau einem PATCH pro tatsächlichem Wechsel und keinem PATCH bei gleicher Auswahl; eine Frontpage steuert die Hardware nur nach ausdrücklicher Kopplung genau ihrer eigenen DemoSession.
