...
## P0.3.2 – vollständig lokal geprüft, zur Veröffentlichung freigegeben
...
- Commit, Push und Deployment der Infrastruktur sind freigegeben; die produktive Verifikation wird nach dem Rollout ergänzt. Kein Production-Seed, kein Händler produktiv angelegt und keine Hardware auf Merchant-API umgestellt; Marketing-Demo bleibt separat. Vor späterem Betrieb bleiben Pepper-/Credential-Provisionierung, lokale WLAN-Konfiguration und reale Merchant-TFT-Abnahme erforderlich. Die Implementierung unterstützt derzeit EUR/USD/GBP/CHF/JPY/KWD/BHD und die im Parser festgelegten Größenlimits; kein vollständiger Merchant-Checkout.
