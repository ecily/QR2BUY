#pragma once

// GlobalSign ECC Root CA R4, the trust anchor currently used by qr2buy.com.
// Public CA certificate; expires 2038-01-19. Keep this under version control.
static const char QR2BUY_ROOT_CA[] PROGMEM = R"CERT(
-----BEGIN CERTIFICATE-----

UM/I2O2dgq43mLyjj0xMqTQrbO/7lZsm
-----END CERTIFICATE-----
)CERT";
